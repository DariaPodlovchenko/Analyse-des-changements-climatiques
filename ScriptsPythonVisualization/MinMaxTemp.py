...
data_file = 'meantemperatures.geojson'  
...
mean_TN_data = gdf.dropna(subset=['mean_TN'])
lon_TN, lat_TN, values_TN = mean_TN_data.geometry.x, mean_TN_data.geometry.y, mean_TN_data['mean_TN']

fig_TN = go.Figure()
fig_TN.add_trace(go.Scattermapbox(
    lon=lon_TN,
    lat=lat_TN,
    mode='markers',
    marker=dict(
    ...
    hovertext=[f"Température: {value:.1f}°C<br>Coordonnées: ({lon:.2f}, {lat:.2f})" 
               for value, lon, lat in zip(values_TN, lon_TN, lat_TN)],
    hoverinfo="text"
))

add_europe_boundaries(fig_TN, gdf_europe)
add_capitals(fig_TN, european_capitals)
...
fig_TN.update_layout(
    ...
    title={
        "text": "Moyenne des Températures journalières minimales en Europe (1950-2023)",  
        "x": 0.5,  
...
fig_TN.write_html("mean_min_temp_map.html")

mean_TX_data = gdf.dropna(subset=['mean_TX'])
lon_TX, lat_TX, values_TX = mean_TX_data.geometry.x, mean_TX_data.geometry.y, mean_TX_data['mean_TX']

fig_TX = go.Figure() 
fig_TX.add_trace(go.Scattermapbox(
    lon=lon_TX,
    lat=lat_TX,
    mode='markers',
    marker=dict(
    ...
    showlegend=False,
    hovertext=[f"Température: {value:.1f}°C<br>Coordonnées: ({lon:.2f}, {lat:.2f})" 
               for value, lon, lat in zip(values_TX, lon_TX, lat_TX)],
    hoverinfo="text"
))

add_europe_boundaries(fig_TX, gdf_europe)
add_capitals(fig_TX, european_capitals)
...
fig_TX.update_layout(
    ...
    title={
        "text": "Moyenne des Températures journalières maximales en Europe (1950-2023)", 
        "x": 0.5,  
        "font": {"family": "Montserrat"}  
    },
    margin=dict(
...
fig_TX.write_html("mean_max_temp_map.html")
